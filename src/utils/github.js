...
export async function openGitHubPRInBrowser(
	...
	debug(
		`openGitHubPRInBrowser: repoUrl=${repoUrl}, branch=${currentBranch}, base=${baseBranch}`,
	);
...
export async function createGitHubPRWithCLI(
	...
	debug(
		`createGitHubPRWithCLI: branch=${currentBranch}, base=${baseBranch}, self=${argv.self}, draft=${argv.draft}`,
	);
	try {
		...
		try {
			...
			if (existingPr) {
				...
				try {
					...
					if (
						editError.message.includes("Projects (classic) is being deprecated")
					) {
						console.log(
						...
						console.log(
							"1. Update the PR description manually in the GitHub web interface",
						);
						...
						console.log(
							"The PR exists but we couldn't update it. You may need to update it manually.",
						);
					}
			...
			if (error.message.includes("Projects (classic) is being deprecated")) {
				...
				console.log(
					"This may prevent checking for existing PRs. The tool will try to create a new PR,",
				);
				console.log(
					"but if a PR already exists, it will fail. You may need to:",
				);
		...
		debug(
			`createGitHubPRWithCLI top-level error: ${error.message}\n${error.stack}`,
		);
